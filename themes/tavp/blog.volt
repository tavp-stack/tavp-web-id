...
<section class="py-24 max-w-[1280px] mx-auto px-gutter">
  <div class="space-y-8">
    <div class="space-y-4">
      <h1 class="font-headline-xl text-headline-xl text-on-surface">Blog</h1>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {% for post in posts %}
          <a href="/blog/{{ post['slug'] }}" class="block bg-surface-container border border-outline-variant rounded-xl p-6 hover:border-secondary transition-colors group">
            ...
            {% endif %}
            <div class="space-y-3">
              <h2 class="font-headline-lg text-lg text-on-surface group-hover:text-secondary transition-colors">{{ post['title'] }}</h2>
              ...
              <div class="flex items-center gap-2 text-xs text-on-tertiary-container">
                {% if post['published_at'] is defined and post['published_at'] %}
                  <span class="font-code-sm">{{ post['published_at'] | date('d M Y') }}</span>
                {% endif %}
              </div>
            </div>
          </a>
